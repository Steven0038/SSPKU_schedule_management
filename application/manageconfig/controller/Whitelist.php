...
//          dump($data);
 ...
 //               $exist_white_list = $excelData->exist_white_list($data['work_id']);
          ...
          else if($excelData->exist_work_id($data['work_id'])){
            $flag=1;
            $failData[$j]=$sqlData[$i];
            $j++;
          }
          else if($excelData->exist_white_list($data['work_id'])){
            $flag=1;
            $lfailData[$l]=$sqlData[$i];
            $l++;
          }
        }
//      dump($failData);
//      dump($lfailData);
      ...
      {echo '因不存在该工号未添成功加人员';
       		echo '<br>';
        for($k=0;$k<$j;$k++){
          echo $failData[$k]['work_id'];
       		echo '<br>';
        }
       echo '因白名单已经存在未添成功加人员：：';
       		echo '<br>';
        for($m=0;$m<$l;$m++){
          echo $lfailData[$m]['work_id'];
       		echo '<br>';
        }
 
      }
      else  $this->success('添加成功,自动跳转');
    }
