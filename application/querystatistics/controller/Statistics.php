...
class Statistics extends Common
{
    ...
    {
				$list = DB::query("SELECT distinct user_info.id id,user_info.name name,depart_id,position_id
									FROM user_info
									where is_delete=0");
				foreach ($list as $key => $value) {
					$depart_exist=Db::table('user_depart')->where('id',$list[$key]['depart_id'])->where('is_delete',0)->find();
					if($depart_exist==null){
						$list[$key]['depart_name']='无部门';
					}else{
						$list[$key]['depart_name']=Db::table('user_depart')->where('id',$list[$key]['depart_id'])->where('is_delete',0)->value('name');
					}


					$position_exist=Db::table('user_position')->where('id',$list[$key]['position_id'])->where('is_delete',0)->find();
					if($position_exist==null){
						$list[$key]['user_posname']='无职位';
					}else{
						$list[$key]['user_posname']=Db::table('user_position')->where('id',$list[$key]['position_id'])->where('is_delete',0)->value('name');
					}
				}
