...
class Index extends Controller
{
    ...
    //  return $this->fetch();
        ...
        if($accessToken){
            ...
            if(!$res){
                return $this->redirect('Index/alarm');
            } else {
                $userid = $this->getUserId($userInfo['card_number']);
                //增加部门和角色在微信端
                $model =model('wxcampus/Index');
                $userInformationById= $model->getDepartAndPosition($userid);
                $this->assign('userInformationById',$userInformationById);
                //$this->assign("userInformationById",$userInformationById);
...
/*        if (curl_errno($ch)) {
    ...
    public function getOpenId($wxcode){
        //第一步:取得openid
        $oauth2Url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=".$this->APP_KEY."&secret=".$this->APP_SECRET."&code=".$wxcode."&grant_type=authorization_code";
        $oauth2 = $this->getJson($oauth2Url); //getJson是自定义的函数
        dump($oauth2);
        //第二步:根据全局access_token和openid查询用户信息
        $access_token = $oauth2["access_token"];
        $openid = $oauth2['openid'];
        $get_user_info_url = "https://api.weixin.qq.com/sns/userinfo?access_token=$access_token&openid=$openid&lang=zh_CN";
        $userinfo = $this->getJson($get_user_info_url);  //getJson的函数定义在第二步
