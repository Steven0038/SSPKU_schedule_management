...
class CalendarValidator extends Validate
{
    ...
    protected $message = [
        ...
        'time_id.checkUniqueTime' => '当前时间段已占用，请选择其他时间段！'
    ];
...
class WxCalendar extends Controller
{
    ...
    protected function isCreateUniqueTime($userid, $timeid, $date){
        $res = Db::name('schedule_info')
            ->where('is_delete', 0)
            ->where('user_id', $userid)
    ...
    protected function isUpdateUniqueTime($userid, $timeid, $id, $date){
        //找出给定用户在给定日程的日程
        $res = Db::name('schedule_info')
        ->where('is_delete', 0)
        ->where('user_id', $userid)
        ...
        if($origin == NULL){//说明在新的一天，相当于在那天新增日程
            // var_dump($res);
            foreach($res as $sched){
                if($sched['time_id'] == $timeid){
                    return false;//与那天的日程冲突
                }
            }
            return true;//没有冲突
        }
        //有当前的日程
        $originTimeId = $origin['time_id'];
        if($originTimeId == $timeid){//1.更新时如果不更新时间段：直接跳过
            return true;
        }else{//2.如果更新时间段：检查是否与其他的相同
            foreach($res as $sched){
                if($sched['time_id'] == $timeid && $sched['id'] != $id){
                    return false;
                }
            }
        }
        return true;//没找到与原来id不同且时间段相同的schedule，返回正确
    }
    ...
    public function create(){
        ...
        //检查是否有重复时间段；
        if(!$this->isCreateUniqueTime($data['user_id'], $data['time_id'],$data['date'])){
            return $this->json('create', false, '当前时间段已被占用，请选择其他时间段！');
        }
        //插入
    ...
    public function update(){
        ...
        //检查是否有重复时间段；
        if(!$this->isUpdateUniqueTime($data['user_id'], $data['time_id'], $data['id'], $data['date'])){
            return $this->json('update', false, '当前时间段已被占用，请选择其他时间段！');
        }
        //找到修改了的参数
    ...
    public function Index(){
        ...
        if($bypass != NULL){
            $this->assign([
                'left' => url('index', ['userid'=>$userid, 'date'=> date('Y-m-d',strtotime($date)-7*24*60*60), 'view'=>'1']),
                'right' => url('index', ['userid'=>$userid, 'date'=> date('Y-m-d',strtotime($date)+7*24*60*60), 'view'=>'1']),
            ]);
    ...
    protected function getScheduleDisplayArray($timestamp){
        ...
        for($tmpTimestamp = $leftTimestamp; $tmpTimestamp <= $rightTimestamp; $tmpTimestamp += 24*60*60){
            ...
            if($tmpTimestamp < $curTimestamp){//跳过过去时间
                continue;
            }
